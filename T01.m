    ...
    if i>1
        ...
        if P_n(i-1)-P_n(i)==1||bool
            ...
            %% Analyze the particle size at the moment of contact
                ...
                V1.Droplet_Radii_T0=radiid(1)*pixcelsize;
                V1.Ball_Area_T0=pi*(radii(1)*pixcelsize)^2;
                V1.Ball_Radii_T0=radii(1)*pixcelsize;            
...
imBfL=bwlabel(imBf);
figure,imshow(imBfL,[]);
BW=imBfL==imBfL(round(centers(1,2)),round(centers(1,1)));
figure,imshow(BW);
bwb=bwboundaries(BW);
...
predicted_centers_b=bwb((sum((bwb-predicted_centers(1,2:-1:1)).^2,2).^0.5-radii(1))<5,:);
% figure,plot(predicted_centers_b(:,1),predicted_centers_b(:,2),'r+')
% figure,plot(bwb(:,1),bwb(:,2),'r+')
...
[~,IE]=sort(abs(Cerror));
predicted_centers_b2=predicted_centers_b(IE(1:end-round(length(IE)/4)),:);
